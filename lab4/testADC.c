...
ISR(TIMER0_OVF_vect){
    ...
    uint8_t adc_rd = read_adc();
    LED[0] = LED_seg(mode, adc_rd, ALARM[MINUTES], 0);
    LED[1] = LED_seg(mode, adc_rd, ALARM[MINUTES], 1);
    ...
    LED[3] = LED_seg(mode, adc_rd, ALARM[HOURS], 3);
